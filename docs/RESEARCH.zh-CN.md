...
## 8. v1 补充：富渲染架构（图片、真表格、主题化）

**状态：两个平台都已实现。** 这是 v0 交付之后，针对"全面支持 Markdown、真表格（不是
兜底）、带缓存的图片加载、可配置颜色/文本主题"这个需求追加的设计，替代了第 7 节
7.4/7.6 里"表格/图片留到 v1 再说"的说法。

**`THKMDView` 从"单一文本视图"变成"分段的纵向容器"。** 真正能横向滚动的表格需要一个
拥有独立触摸事件处理的视图——这是 `Span`/`NSTextAttachment` 从根本上做不到的，因为它们
只是画在某个文本视图自己画布里的内容，不是能独立命中测试（hit-test）的视图。所以渲染
现在会遍历顶层块级语法树，产出一串"段（segment）"：

- **文本段** —— 和第 3/4 节里的做法一致：尽可能多的连续非表格块（段落、标题、列表、
  引用、代码块、分割线）依然会合并成一个 `Spanned`/`NSAttributedString`，由一个内部文本
  视图渲染。大多数消息（不含表格）依然只会渲染成一个段——第 3 节里"单一视图性能更好"
  的论证在常见场景下继续成立。
- **表格段** —— 新增一个专门的视图（两端都叫 `THKTableView`）：真正的列布局、独立于
  消息气泡的横向滚动、主题化的边框/表头颜色；因为 GFM 表格单元格里可以包含行内
  Markdown，每个单元格的文本都会走和别处一样的行内 span/属性构建逻辑，而不是简化成
  纯文本。
- 每次防抖渲染都会重建这一串段（还是第 4 节里"缓冲区 + 防抖 + 整体重新解析"那套流式
  模型——只是渲染这一步的*输出形态*变了）。重建时会按 `(下标, 类型)` 去匹配/复用已有的
  段视图，避免无意义的销毁重建；v1 阶段不要求做到完美的最小化 diff。

**图片依然内联在文本流里**（不单独占一段）：用一个自定义的异步加载 span/attachment，
先预留一个固定尺寸的占位框（真实图片到达后不会引起周围文字重新排版——图片会被缩放
以适配这个预留框），加载完成后替换成真实的位图/图片，只重绘这一处。

**可插拔的图片加载**：`THKImageLoader`（Android 接口）/`THKImageLoading`（iOS
protocol）——只有一个 `load(url) -> Bitmap?/UIImage?` 方法。自带一个不依赖第三方库的
默认实现（内存 LRU + 磁盘缓存，基于 `HttpURLConnection`/`URLSession`——刻意不用
Coil/Kingfisher，一是不想给 SDK 强加一个第三方依赖，二是避免重蹈之前 swift-markdown
那种 SPM/CocoaPods 分发不一致的覆辙）。`THKMDView.imageLoader` 可以整体替换，所以已经
在用 Coil/Kingfisher/Glide 的宿主 App 可以直接换成自己的适配实现。

**主题化**：一个不可变的 `THKMDTheme` 值类型（正文/标题/链接/代码文本+背景/引用竖线/
表格边框+表头等颜色，以及字号），可以设置在 `THKMDView` 上，默认值和当前的视觉效果
保持一致。

**接受的破坏性变更**：`THKMDView` 不再直接是 `AppCompatTextView`/`UITextView` 的子类
——它变成了一个容器型的 `ViewGroup`/`UIView`，内部持有一个或多个文本/表格子视图。现在
改是合适的时机，因为 SDK 目前还没有外部使用方（仍处于 pre-1.0 阶段）；这里特意记录
一下，免得以后被误以为是遗漏。

### 实现落地情况（实际交付内容，以及已知的 v1 缺口）

- 可插拔的 `MarkdownRenderer`/`MarkdownRendering` 接口里 `render(...)` 方法的签名
  必然发生了变化（现在返回的是一串 segment，Android 上还多带了当前的
  `theme`/图片尺寸配置）——v0→v1 之间只保证了 `THKMDView` 自己对外的方法签名逐字不变。
- 图片点击：Android 新增了专门的 `onImageClick`（不设置的话会退回用图片 URL 调用
  `onLinkClick`）；iOS 在原有 `onLinkTap` 之外新增了 `onImageTap`。
- 嵌套在引用块或列表项里的表格（也就是非顶层块的表格），两端都不会渲染成真表格——
  这是一个实际场景里很少见、目前没做的缺口。
- **仅 iOS/CocoaPods 这条路径的缺口**：Maaku（CocoaPods 这条分发路径用的解析器）不会
  保留 GFM 有序列表"非 1 开头"的起始序号——它的 `OrderedList` 类型虽然接收了起始数字，
  但在 `DocumentConverter` 里直接丢弃了。这是 Maaku 本身的上游限制，SPM/swift-markdown
  这条路径没有这个问题。CocoaPods 那边专门有一个测试断言了实际的（从 1 重新编号的）
  行为，而不是假装它和 SPM 那边的结果一致。
- Android 的图片无障碍描述是设置在整个文本段（`Spanned`）级别的，不是挂在单个图片
  span 上——Android `TextView` 如果不自定义无障碍委托，本来就没有给单个 span 单独暴露
  无障碍节点的机制，v1 阶段判断这个复杂度不值得引入。
- iOS 在补嵌套引用块的测试时，真的抓到并修了一个 bug：外层引用块的竖线颜色/缩进
  会被无条件地盖到整个合并区间上，把内层（更深一层）引用块自己的样式覆盖掉了。修复
  方式是统一改成"这个属性已经设置过就不再覆盖"的合并规则。
