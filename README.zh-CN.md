...
| iOS 工具链 | 当前源码/二进制构建使用 Xcode 26+ / Swift 6.2+；旧工具链兼容性需按分发版本验证 |
...
**1.0.2** 起库已内置 consumer rules，业务无需手动添加本库混淆规则。
...
解析器固定到 commit，目前请按分支或 commit 接入，不支持版本号规则。
...
示例应用提供格式用例、30 组模拟 AI 回复、科技周刊阅读与可持久化主题编辑页面。示例业务代码和数据不包含在库中。

运行示例或参与开发：[Android 开发指南](android/README.md) / [iOS 开发指南](ios/README.md)。
